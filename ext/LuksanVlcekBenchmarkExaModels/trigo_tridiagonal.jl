@inline function LV.trigo_tridiagonal_model(::LV.ExaModelsBackend, N = 1000; T = Float64, backend = nothing, prod = false, kwargs...)
    n = N
    c = EM.ExaCore(T; backend = backend, kwargs...)
    EM.@var(c, x, N; start = fill(1, N))
    EM.@con(c, LV.trigo_tridiagonal_con1(x))
    EM.@con(c, LV.trigo_tridiagonal_con2(x))
    EM.@con(c, LV.trigo_tridiagonal_con_n1(x, n))
    EM.@con(c, LV.trigo_tridiagonal_con_n(x, n))
    EM.@obj(c, LV.trigo_tridiagonal_objective(x, i) for i = 2:N-1)
    return EM.ExaModel(c; prod = prod)
end
