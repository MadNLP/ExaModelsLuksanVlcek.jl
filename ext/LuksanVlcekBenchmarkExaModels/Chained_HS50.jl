@inline function LV.Chained_HS50_model(::LV.ExaModelsBackend, N = 1000; T = Float64, backend = nothing, prod = false, kwargs...)
    ...
    EM.@var(c, x, N; start = (LV.Chained_HS50_start(i) for i = 1:N))
    EM.@con(c, LV.Chained_HS50_con1(x, l) for l in It_L1)
    EM.@con(c, LV.Chained_HS50_con2(x, l) for l in It_L2)
    EM.@con(c, LV.Chained_HS50_con3(x, l) for l in It_L3)
    EM.@obj(c, LV.Chained_HS50_objective(x, i) for i in 1:floor(Int, (N-1)/4))
