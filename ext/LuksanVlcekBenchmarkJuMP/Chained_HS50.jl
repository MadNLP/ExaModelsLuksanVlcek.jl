@inline function LV.Chained_HS50_model(::LV.JuMPBackend, N = 1000; optimizer = nothing, kwargs...)
    ...
    JuMP.@constraint(m, [l in It_L1], LV.Chained_HS50_constraint1(x, l) == 0)
    JuMP.@constraint(m, [l in It_L2], LV.Chained_HS50_constraint2(x, l) == 0)
    JuMP.@constraint(m, [l in It_L3], LV.Chained_HS50_constraint3(x, l) == 0)
