...
cragg_levy_start(i) = mod(i, 4) == 1 ? 1.0 : 2.0
cragg_levy_constraint(x, k) = 8 * x[k+1] * (x[k+1]^2 - x[k]) - 2 * (1 - x[k+1]) + 4 * (x[k+1] - x[k+2]^2)
cragg_levy_objective(x, i) = (exp(x[2i-1]) - x[2i])^4 + 100 * (x[2i] - x[2i+1])^6 +
