...
Chained_HS50_start(i) = mod(i, 4) == 1 ? 35.0 : mod(i, 4) == 2 ? 11.0 : mod(i, 4) == 3 ? 5.0 : -5.0
Chained_HS50_con1(x, l) = x[l+1]^2 + 2 * x[l+2] + 3 * x[l+3]^2 - 6
Chained_HS50_con2(x, l) = x[l+2]^2 + 2 * x[l+3] + 3 * x[l+4] - 6
Chained_HS50_con3(x, l) = x[l+3]^2 + 2 * x[l+4] + 3 * x[l+5] - 6
Chained_HS50_objective(x, i) = (x[4(i-1)+1] - x[4(i-1)+2])^2 + (x[4(i-1)+2] - x[4(i-1)+3])^2 +
