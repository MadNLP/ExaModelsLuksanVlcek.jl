...
Chained_HS49_start(i) = mod(i, 3) == 1 ? 10.0 : mod(i, 3) == 2 ? 7.0 : -3.0
Chained_HS49_con1(x, l) = x[l+1]^2 + x[l+2] + x[l+3] + 4 * x[l+4] - 7
Chained_HS49_con2(x, l) = x[l+3]^2 - 5 * x[l+5] - 6
Chained_HS49_objective(x, i) = (x[3(i-1)+1] - x[3(i-1)+2])^2 + (x[3(i-1)+3] - 1)^2 +
