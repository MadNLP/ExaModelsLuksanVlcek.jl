...
Chained_HS51_start(i) = mod(i, 4) == 1 ? 2.5 : mod(i, 4) == 2 ? 0.5 : mod(i, 4) == 3 ? 2.0 : -1.0
Chained_HS51_con1(x, l) = x[l+1]^2 + 3 * x[l+2] - 4
Chained_HS51_con2(x, l) = x[l+3]^2 + 2 * x[l+4] - 2 * x[l+5]
Chained_HS51_con3(x, l) = x[l+2]^2 - x[l+5]
Chained_HS51_objective(x, i) = (x[4(i-1)+1] - x[4(i-1)+2])^4 + (x[4(i-1)+2] + x[4(i-1)+3] - 2)^2 +
