...
# The index in nC would be out of range for the constraints mod. Threfore, the last term in each constraint are not implemented. The number of constraints fulfilled the question. 
function Chained_HS51_model(N = 1000; T = Float64, backend=nothing, kwargs ...)
    nC = (3 * (N-1) ÷ 4)
    ...
    ExaModels.objective(c, (x[4(i-1)+1] - x[4(i-1)+2])^4 + (x[4(i-1)+2] + x[4(i-1)+3] - 2)^2 + (x[4(i-1)+4] - 1)^2 + (x[4(i-1)+5] - 1)^2 for i in 1:floor(Int, (N-1)/4))
